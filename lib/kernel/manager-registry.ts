...
export const MAINTENANCE_DOMAINS: Record<string, { manager: ManagerKey; proof: string; what: string }> = {
  ...
  document_compliance_audit:  { manager: "deal_coordinator", proof: "test:document-audit", what: "DOCUMENT-VISION COMPLIANCE AUDIT (retires audit-document + broker-audit workflows) — when a document is UPLOADED to a deal, an AI vision pass scans it (missing signatures/dates/initials, wrong/expired form, missing disclosures, mismatched names/addresses), classifies findings deterministically, records the result on client_documents.ai_metadata (REUSED jsonb, no new table), and on findings ESCALATES to the broker via the existing notifications rail + a consumed manager-signals bus line. Covers DOCUMENTS clients/agents upload (the text gate covers text WE generate). Vision is an injectable seam; honest 'not_audited' when creds absent — NEVER fabricates a finding or a pass; idempotent per (document, vision content); hourly sweep cron" },
  document_autofile:          { manager: "deal_coordinator", proof: "test:document-autofile", what: "THE AI AUTO-FILING CLERK (retires document-storage-logic workflow) — kills the #1 transaction-admin chore: a recently-uploaded UNFILED client_documents row is classified (injectable gateway classifier → PURE keyword/extension floor) into the canonical doc_category enum and routed to the right DEAL (stamped transaction, else the uploader's single open transaction) — sets doc_category + document_type folder key + transaction_id + ai_metadata audit, recorded on lifecycle_events. HONEST: low confidence → left UNFILED + flagged for human (never misfiled); idempotent (an already-categorized doc is a no-op). In-app canonical filing (no Drive API wired → Drive sync is a documented follow-up); 15-min sweep cron" },
