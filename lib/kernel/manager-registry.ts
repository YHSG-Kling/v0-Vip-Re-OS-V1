...
export const MAINTENANCE_DOMAINS: Record<string, { manager: ManagerKey; proof: string; what: string }> = {
  ...
  listing_inventory_radar:    { manager: "data_steward", proof: "test:inventory-radar", what: "Continuously surfaces SELLER-INTENT candidates the scraper bench already scraped (expired/withdrawn, FSBO, absentee, high-equity/pre-foreclosure) from raw_scraped_leads, scores intent 0..1 (documented weights — pre-foreclosure+high-equity outranks a fresh low-equity FSBO) + ranks, and routes the hottest data_steward → listing_concierge; the handler proposes a GATED 'thinking of selling?' brief + a Director-commissioned reel + flags a gated CMA; idempotent per (candidate, window); never scrapes, never fabricates contact info, nothing auto-sends" },
}
