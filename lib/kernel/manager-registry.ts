...
export const MAINTENANCE_DOMAINS: Record<string, { manager: ManagerKey; proof: string; what: string }> = {
  ...
  document_compliance_audit:  { manager: "deal_coordinator", proof: "test:document-audit", what: "DOCUMENT COMPLIANCE AUDIT (retires audit-document + broker-audit workflows) — a TEXT-extraction pass (the existing lib/documents/ocr-pdf text layer, NO vision) scans an uploaded deal document against a deterministic + optional gateway-TEXT checklist (signature-execution markers, real execution dates, required disclosures, right form/type for the transaction stage), records the result on client_documents.ai_metadata (REUSED jsonb, no new table), and on findings ESCALATES to the broker via the existing notifications rail + a consumed manager-signals bus line. Covers DOCUMENTS clients/agents upload (the text gate covers text WE generate). Text-extraction is an injectable seam; honest 'not_audited' when no text layer can be extracted — NEVER fabricates a finding or a pass; idempotent per (document, extracted-text content); hourly sweep cron" },
