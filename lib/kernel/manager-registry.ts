...
export const MAINTENANCE_DOMAINS: Record<string, { manager: ManagerKey; proof: string; what: string }> = {
  ...
  seller_appt_conversion:     { manager: "ai_isa",       proof: "test:seller-appt-conversion", what: "The ISA owns lead→appointment→conversion: booking a qualified seller LEAD schedules the listing appointment, CONVERTS lead→contact (canonical lossless converter, agent assigned → portal + welcome/meet-your-agent intro via CONTACT_AGENT_ASSIGNED fired synchronously), and fires listing.appointment_set → the flagship listing-appt-prep chain; ordered (contact before chain), idempotent (no double convert/book/fire), honest-guarded (lead under representation → refuse)" },
