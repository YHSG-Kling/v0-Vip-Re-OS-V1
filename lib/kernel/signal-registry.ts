...
export const SIGNAL_REGISTRY: Record<string, SignalSpec> = {
  ...
  scraper_source_failed:   { consumers: ["data_steward"],                          disposition: "handled", kind: "alert",      what: "a scheduled lead-scraping source failed to connect N times in a row (network / 403 / timeout / actor down) — the Data Steward (now owner of the SCRAPING half of the spine, not just raw→contact) escalates a HIGH alert to the brokerage broker/admins so a dead source can't silently dry up the lead funnel" },
