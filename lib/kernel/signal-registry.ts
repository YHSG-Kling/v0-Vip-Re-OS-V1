...
export const SIGNAL_REGISTRY: Record<string, SignalSpec> = {
  ...
  buyer_target_risk:       { consumers: ["deal_coordinator"],                     disposition: "handled", kind: "update",     what: "the deal-killer radar flagged HIGH/CRITICAL red flags on a buyer's TARGET property before any offer — the Shopping Agent gave the agent the gated heads-up AND hands the same flag to the Deal Coordinator, which pre-warns the deal-coordination owners (the TC side) so title/lien/insurability issues surface before acceptance, not after; TC + agent both in the loop" },
