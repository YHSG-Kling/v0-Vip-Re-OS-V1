/**
 * lib/intelligence/quarterly-review-loader.ts
 *
 * THE ONE QBR LOADER — every trailing-90d count the Quarterly Business
 * Review + adoption pulse stand on, shared by the dashboard action AND
 * the spoken quarterly_review verb (keep-one rule: two front-ends, one
 * loader, no drift). Callers gate access (tier-parity principal);
 * this only loads + composes.
 */
...
export async function loadQuarterlyReview(svc: Svc, brokerageId: string): Promise<{ review: QuarterlyReview; pulse: PulseEntry[] }> {
  const now = new Date()
