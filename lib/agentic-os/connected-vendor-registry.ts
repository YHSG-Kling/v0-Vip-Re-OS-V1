...
export type ConnectedCapability =
  ...
  | "payment_transfer"          // move funds / commission payout (financial)
  | "accounting_sync"           // sync an invoice/journal entry to accounting
...
export type ConnectedDomain =
  | "listings" | "crm" | "communications" | "scheduling" | "finance"
...
export const CONNECTED_CAPABILITY_REGISTRY: Record<ConnectedCapability, ConnectedCapabilityDef> = {
  ...
  crm_contact_sync:          { capability: "crm_contact_sync",          verb: "UPDATE",  scope: "crm:write",         domain: "crm",            mutates: true,  purpose: "Push or sync a contact record to the brokerage's connected CRM.", inputs: ["contactId"], connections: ["gohighlevel"] },
  email_send:                { capability: "email_send",                verb: "NOTIFY",  scope: "comms:email",       domain: "communications", mutates: true,  purpose: "Send an email through the brokerage's connected email provider.", inputs: ["to", "subject", "body"], connections: ["sendgrid", "gmail", "outlook"] },
  sms_send:                  { capability: "sms_send",                  verb: "NOTIFY",  scope: "comms:sms",         domain: "communications", mutates: true,  purpose: "Send an SMS through the connected phone provider (TCPA-gated).", inputs: ["to", "body"], connections: ["twilio"] },
  phone_call_place:          { capability: "phone_call_place",          verb: "NOTIFY",  scope: "comms:voice",       domain: "communications", mutates: true,  purpose: "Place an outbound phone call via the connected phone provider (TCPA-gated).", inputs: ["to"], connections: ["twilio"] },
  calendar_event_book:       { capability: "calendar_event_book",       verb: "BOOK",    scope: "calendar:write",    domain: "scheduling",     mutates: true,  purpose: "Create an event on the brokerage's connected calendar.", inputs: ["title", "startsAt", "endsAt", "attendees?"], connections: ["nylas"] },
  calendar_availability_get: { capability: "calendar_availability_get", verb: "GET",     scope: "calendar:read",     domain: "scheduling",     mutates: false, purpose: "Read free/busy availability from the connected calendar.", inputs: ["from", "to"], connections: ["nylas"] },
  payment_transfer:          { capability: "payment_transfer",          verb: "CREATE",  scope: "finance:write",     domain: "finance",        mutates: true,  purpose: "Move funds / commission payout via the connected financial provider.", inputs: ["amount", "currency", "destination"], connections: ["stripe"] },
  accounting_sync:           { capability: "accounting_sync",           verb: "UPDATE",  scope: "finance:write",     domain: "finance",        mutates: true,  purpose: "Sync an invoice or journal entry to the connected accounting system.", inputs: ["entityType", "entityId"], connections: ["quickbooks", "stripe"] },
  ...
  social_account_publish:    { capability: "social_account_publish",    verb: "PUBLISH", scope: "social:connected",  domain: "social",         mutates: true,  purpose: "Publish a post to a connected social account.", inputs: ["assetId", "channels"], connections: ["meta", "linkedin", "twitter", "tiktok", "youtube", "pinterest"] },
