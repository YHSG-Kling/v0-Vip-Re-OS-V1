...
export function payloadLeaksSecret(payload: SnapshotPayload): boolean {
  const layers = [payload.global, payload.brand, payload.voice]
  return layers.some((l) => l && SNAPSHOT_FORBIDDEN_FIELDS.some((f) => f in (l as object)))
}
...
export async function buildSnapshotPayload(sourceBrokerageId: string, client?: Svc): Promise<SnapshotPayload> {
  ...
  const [{ data: gs }, { data: bs }, { data: bv }, { data: overrides }] = await Promise.all([
