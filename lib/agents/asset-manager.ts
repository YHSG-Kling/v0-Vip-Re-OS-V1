...
function buildKickoffPrompt(snap: AssetSnapshot): string {
  return [
    ...
    "",
    "  { action_type: 'start_render',",
    "    action_input: { composition_id: '<id from remotion_compositions>', scope_type: 'agent'|'team'|'brokerage', scope_id: '<uuid>', agent_user_id?: '<uuid>', entity_type?: '<string>', entity_id?: '<uuid>' },",
    "    rationale: 'why a fresh render of this composition matters this week — e.g. unused composition gap, market-update freshness, listing-promo cycle.' }",
    "",
    "  { action_type: 'restart_failed_render',",
    "    action_input: { render_id: '<uuid of the failed remotion_composition_renders row>' },",
    "    rationale: 'why the failure is now resolvable — e.g. the missing brand asset has been uploaded; D-ID quota has reset.' }",
    "",
    "  { action_type: 'publish_video_page',",
    "    action_input: { render_id: '<uuid of a SUCCEEDED remotion_composition_renders row>' },",
    "    rationale: 'this render is broker-ready and would earn AI-search/share reach as a public /v/[slug] page (schema.org VideoObject + RealEstateListing). Propose for evergreen listing/market/neighborhood pieces — NOT time-sensitive open-house/coming-soon that expire.' }",
    "",
    "RULES:",
