...
export const WEBHOOK_CONTRACT: WebhookContractEntry[] = [
  // ── Meta (one app shell; tenants connect their own pages/accounts) ─────────
  {
    ...
    secretEnv: ["META_WEBHOOK_VERIFY_TOKEN", "META_VERIFY_TOKEN", "WHATSAPP_VERIFY_TOKEN", "META_APP_SECRET", "FACEBOOK_APP_SECRET"],
    ...
    notes: "Survivor of the meta/meta-dm duplicate pair (2026-08-27) — tombstone in the route header. 2026-09-03: verifier lifted to lib/meta/verify-signature.ts (the ONE Meta verifier, shared with meta-leadgen + whatsapp); META_WEBHOOK_VERIFY_TOKEN is the one handshake token, the other two spellings are accepted fallbacks; unset App Secret now answers 503 (was 200-ack-no-ingest).",
  },
  {
    provider: "meta",
    eventKind: "lead-ads",
    ...
    notes: "GAP CLOSED 2026-09-03: POST is X-Hub-Signature-256-verified through lib/meta/verify-signature.ts (503 secret unset, 401 mismatch) — an unauthenticated body can no longer mint tcpa_consent=true contacts.",
  },
  {
    provider: "meta",
    ...
    secretEnv: ["META_WEBHOOK_VERIFY_TOKEN", "META_VERIFY_TOKEN", "WHATSAPP_VERIFY_TOKEN", "META_APP_SECRET", "FACEBOOK_APP_SECRET"],
    implementedIn: ["lib/meta/verify-signature.ts"],
    protocolVersion: "WhatsApp Cloud API (Graph API v25.0 current, 2026-08-27); signs X-Hub-Signature-256 like every Graph webhook",
    consoleField: "Meta App Dashboard → WhatsApp → Configuration → Webhook Callback URL",
    failureVisibility: null,
    notes: "GAP CLOSED 2026-09-03: POST is X-Hub-Signature-256-verified through lib/meta/verify-signature.ts. META_WEBHOOK_VERIFY_TOKEN is the one handshake token; WHATSAPP_VERIFY_TOKEN / META_VERIFY_TOKEN accepted as fallbacks.",
  },
  ...
  // ── Social DM (non-Meta) ───────────────────────────────────────────────────
  {
    ...
    scheme: "hmac-sha256",
    verificationHeaders: ["x-li-signature"],
