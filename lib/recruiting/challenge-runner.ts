...
export async function scoreChallenge(svc: Svc, challenge: ChallengeRow, agentIds: string[], now: Date): Promise<Participant[]> {
  ...
  try {
    if (type === "most_points") {
      ...
      const { data } = await svc.from("transactions").select("agent_id").in("agent_id", agentIds).eq("status", "closed").gte("close_date", start.slice(0, 10)).lte("close_date", end.slice(0, 10))
      ...
      const { data } = await svc.from("referrals").select("agent_id").in("agent_id", agentIds).gte("created_at", start).lte("created_at", end)
      for (const r of (data ?? []) as any[]) add(r.agent_id, 1)
    }
  ...
  for (const c of (challenges ?? []) as ChallengeRow[]) {
    ...
    if (ending && c.status !== "ended") {
      ...
      if (c.prize_points > 0 && winners.length > 0) {
        await svc.from("agent_points_log").insert(winners.map((w) => ({ agent_id: w.agentId, brokerage_id: params.brokerageId, points: c.prize_points, reason: "CHALLENGE_PRIZE", reference_type: "challenge", reference_id: c.id }))).then(undefined, () => {})
