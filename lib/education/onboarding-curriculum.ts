...
const CORE: OnboardingTopic[] = [
  ...
  // BUYER-REPRESENTATION / FINANCING — the letter-strength conversation (owner directive: agents AND
  // contacts must be trained on pre-qualification vs pre-approval and which letter is the stronger one).
  { key: "financing_letter_strength", label: "Pre-qualification vs pre-approval: coaching buyers to the stronger letter", audienceRoles: ["agent"], brief: "The buyer-representation financing conversation every agent must own. Teach the real difference: a PRE-QUALIFICATION is an estimate built on self-reported numbers — no credit pull, no underwriting, no document review — a soft starting point only; a PRE-APPROVAL is lender-verified (credit pulled, income and asset documents reviewed by underwriting) and is a conditional commitment to lend — the STRONGER letter, and the one listing agents and sellers actually weight (many sellers won't take an offer seriously without one). Cover the COACHING moves with verbatim scripts: (1) how to explain the difference to a buyer in plain language without jargon; (2) how to move a pre-qual buyer to a full pre-approval EARLY — before they fall in love with a home, not the week they want to write; (3) the document checklist a lender needs (pay stubs, W-2s/tax returns, bank statements, ID) so the upgrade takes days, not weeks; (4) that letters EXPIRE — typically 60-90 days — so time the letter (and refreshes) against the search window; and (5) how to present letter strength to a listing agent or seller when submitting — position a verified pre-approval as offer strength, and NEVER overstate what a pre-qualification is (honesty here protects the buyer's credibility and the agent's). Compliant, fair-housing-safe framing throughout: this is about financing process and documentation, never about who the buyer is." },
  { key: "contract_walkthrough", label: "The purchase contract, section by section", audienceRoles: ["agent"], brief: "Walk the standard purchase contract SECTION BY SECTION for an agent who may never have filled one alone: what each section legally means in plain English, exactly what goes in each blank, the mistakes that kill deals (wrong legal names, missed contingency boxes, bad date math), how contingencies interact, and what changes for cash vs financed offers. Depth for a first-year licensee; advanced notes for veterans." },
  { key: "business_operating_rhythm", label: "Running your real-estate business: the operating rhythm", audienceRoles: ["agent", "broker"], brief: "The BUSINESS of real estate, not the sales part: the weekly operating rhythm (prospecting blocks, follow-up debt, deal file hygiene), money management on irregular commission income (taxes set aside, a real P&L, what to reinvest), lead-flow economics (what a closing actually costs to earn), when to hire help, and the numbers a solo agent or team lead must review monthly to run this like a company instead of a job." },
]
