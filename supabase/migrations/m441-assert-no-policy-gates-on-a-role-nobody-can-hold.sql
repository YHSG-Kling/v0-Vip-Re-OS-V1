...
--       49 policies carry 79 such literals — chiefly 'broker_admin' (which
...
-- literal `user_type` occurrence to anchor on. On this schema that under-counts
-- the class by more than a factor of five (12 literals found, 86 present), and it
-- under-counts in the direction that matters: a policy is only WHOLLY dead if
...
-- written, and claim 4 reports it as a warning rather than passing silently,
...
begin
  ...
  loop
    ...
    foreach seg in array (select array_agg(s order by o)
...
begin
  ...
  if n > 0 then
    raise exception
      'm441: % polic(ies) test user_type by EXCLUSION. "Not an agent" is TRUE for a contact, a lender and a vendor — that exact clause is how a client portal read the brokerage''s commission book (m433) — and it admits every role invented later, by default, with nobody editing anything. State who MAY act, never who may not.%s%s',
      n, chr(10), offenders;
  end if;
end $$;
...
-- as "no dead role spellings remain in this schema". Measured at m440: 49
-- policies, 79 literals, concentrated on 'broker_admin' and 'super_admin'.
--
...
-- which is how a guard gets disabled. Both are reported in the wave notes instead.
--
