...
console.log("     clip is LONG ENOUGH for its slot — that is now a SEPARATE guard,")
console.log("     npm run test:broll-slot, and this blind spot is CLOSED rather than open.")
console.log("     (This block described the even division and the dropped plan as live")
console.log("     defects for two waves after they were fixed. A blind-spot note is what a")
console.log("     reader trusts when the number looks clean, so a stale one is worse than")
console.log("     none: selectBrollPlan now runs in the frame domain inside the layer and")
console.log("     each clip's measured duration bounds its own slot.)")
