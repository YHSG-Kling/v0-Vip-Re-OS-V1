  ...
  while (i < s.length) {
    ...
    if (c === "/" && s[i + 1] === "/") {
      ...
      while (i < s.length) {
        ...
        // A single/double-quoted literal cannot span a newline; bail rather than
        // masking the rest of the file when an apostrophe appears in prose the
        // comment rules did not already cover.
        if (q !== "`" && s[i] === "\n") break
        mask[i++] = true
      }
    } else {
