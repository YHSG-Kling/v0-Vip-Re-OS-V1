...
function sourceLayer() {
  ...
  check("the backstop uses the canonical detector + records to compliance_events",
    /detectFairHousingViolations/.test(cs) && /from\("compliance_events"\)/.test(cs) && /fair_housing_dispatch_backstop/.test(cs))
...
async function liveLayer() {
  ...
  try {
    ...
    const { count } = await svc.from("compliance_events").select("id", { count: "exact", head: true }).eq("brokerage_id", b).eq("gate_name", "fair_housing_dispatch_backstop")
