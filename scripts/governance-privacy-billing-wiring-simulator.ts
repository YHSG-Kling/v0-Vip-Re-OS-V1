...
for (const fn of ["getAllBrokeragesBilling", "getDelinquentAccounts"] as const) {
  def({
    id: `billing.${fn}.capability-gate`,
    what: `${fn} is gated by a platform CAPABILITY, with no tenant-role list anywhere in the body`,
    file: F.billing,
    mutate: [
      `export async function ${fn}(`,
      `async function __disabled_${fn}(`,
    ],
    run() {
      const b = fnBody(src(F.billing), fn)
      if (!b) return { ok: false, detail: "function body not found" }
      // CONSTRUCT: a capability gate call whose result is branched on, before any read
      const gateAt = at(b, /requirePlatformCapability\s*\(/)
      const readAt = at(b, /\.from\(/)
      const branched = hasGuardBranch(b, /!\s*\w+\.ok/, /return[\s\S]*ok\s*:\s*false/)
      const roleList = usesTenantRoleList(b)
      // and the gate must be the real one from the canonical module
      const canonical = /requirePlatformCapability/.test(src(F.capGate))
      return {
        ok: gateAt < readAt && branched && !roleList && canonical,
        detail: `gateIdx=${gateAt} firstReadIdx=${readAt} branched=${branched} tenant-role-list=${roleList} canonical-module=${canonical}`,
      }
    },
  })

  // TIGHTENED: anchor the mutation inside THIS function. The first version of
  // these two checks shared one anchor, so mutating getAllBrokeragesBilling left
  // getDelinquentAccounts untouched and its check "passed" a mutation it had
  // never seen. The anchor now includes each function's own destructuring line.
  const denialAnchor = fn === "getAllBrokeragesBilling"
    ? `if (!gate.ok) return { ok: false, error: gate.error ?? "Forbidden" }\n\n  const supabase = await createClient()\n  // A refused`
    : `if (!gate.ok) return { ok: false, error: gate.error ?? "Forbidden" }\n\n  const supabase = await createClient()\n  const { data: delinquent`
  const denialAnchorMutated = denialAnchor.replace(
    `if (!gate.ok) return { ok: false, error: gate.error ?? "Forbidden" }`,
    `if (!gate.ok) return { ok: true, rows: [] }`,
  )
  def({
    id: `billing.${fn}.denial-is-not-empty`,
    what: `${fn} reports a refusal as a refusal, never as an empty roster`,
    file: F.billing,
    mutate: [denialAnchor, denialAnchorMutated],
    run() {
      const b = fnBody(src(F.billing), fn)
      if (!b) return { ok: false, detail: "function body not found" }
      const deniesLoudly = hasGuardBranch(b, /!\s*\w+\.ok/, /ok\s*:\s*false/)
      // TIGHTENED: no path in this body may hand back a fabricated empty roster.
      const fabricatesEmpty = /return\s*\[\s*\]/.test(b) || /rows\s*:\s*\[\s*\]/.test(b)
      const gateBeforeRead = at(b, /!\s*\w+\.ok/) < at(b, /\.from\(/)
      return {
        ok: deniesLoudly && !fabricatesEmpty && gateBeforeRead,
        detail: `denies-loudly=${deniesLoudly} fabricated-empty-roster=${fabricatesEmpty} denial-before-read=${gateBeforeRead}`,
      }
    },
  })
}
...
def({
  ...
  what: "manualTierOverride requires the billing capability with WRITE and a resolved superadmin role",
  file: F.billing,
  mutate: ['if (gate.role !== "superadmin") {', "if (false) {"],
  run() {
    const b = fnBody(src(F.billing), "manualTierOverride")
    if (!b) return { ok: false, detail: "function body not found" }
    const requiresWrite = /requirePlatformCapability\s*\([^)]*requireWrite\s*:\s*true/.test(b)
    const superOnly = hasGuardBranch(b, /\.role\s*!==\s*["']superadmin["']/, /return[\s\S]*ok\s*:\s*false/)
    const roleList = usesTenantRoleList(b)
    const gateBeforeWrite = at(b, /requirePlatformCapability\s*\(/) < at(b, /\.update\(/)
    return {
      ok: requiresWrite && superOnly && !roleList && gateBeforeWrite,
      detail: `requireWrite=${requiresWrite} superadmin-branch=${superOnly} tenant-role-list=${roleList} gate-before-write=${gateBeforeWrite}`,
    }
  },
})
...
def({
  ...
  file: F.billing,
  mutate: ["if (trimmedReason.length < 10) {", "if (false) {"],
  run() {
    const b = fnBody(src(F.billing), "manualTierOverride")
    const reasonGuard = hasGuardBranch(b, /trimmedReason[\s\S]*length/, /return[\s\S]*ok\s*:\s*false/)
    const guardBeforeWrite = at(b, /trimmedReason\s*\.?\s*length/) < at(b, /\.update\(/)
    const payload = writePayload(b, "audit_log", "insert")
    const who = /\buser_id\s*:/.test(payload)
    const why = /\breason\s*:/.test(payload)
    const beforeAfter = /\bbefore\s*:/.test(payload) && /\bafter\s*:/.test(payload)
    return {
      ok: reasonGuard && guardBeforeWrite && who && why && beforeAfter,
      detail: `reason-guard=${reasonGuard} before-write=${guardBeforeWrite} audit.user_id=${who} audit.reason=${why} before+after=${beforeAfter}`,
    }
  },
})
...
def({
  ...
  file: F.billing,
  mutate: ["if (previousTierId === newTierId) {", "if (false) {"],
  run() {
    const b = fnBody(src(F.billing), "manualTierOverride")
    const branch = hasGuardBranch(b, /previousTierId\s*===\s*newTierId/, /return[\s\S]*ok\s*:\s*true/)
    const beforeWrite = at(b, /previousTierId\s*===\s*newTierId/) < at(b, /\.update\(/)
    const beforeAudit = at(b, /previousTierId\s*===\s*newTierId/) < at(b, /\.from\(\s*["']audit_log["']/)
    return { ok: branch && beforeWrite && beforeAudit, detail: `no-op-branch=${branch} before-update=${beforeWrite} before-audit=${beforeAudit}` }
  },
})

def({
  id: "billing.tier-override.zero-row-refusal",
  what: "a tier update that matched no subscription is reported as a failure",
  file: F.billing,
  mutate: ["if ((count ?? 0) === 0) {\n    // An UPDATE matching nothing still succeeds in postgrest.", "if (false) {\n    // An UPDATE matching nothing still succeeds in postgrest."],
  run() {
    const b = fnBody(src(F.billing), "manualTierOverride")
    const counted = /\.update\(\s*\{[\s\S]*?\}\s*,\s*\{\s*count\s*:\s*["']exact["']/.test(b)
    const branch = hasGuardBranch(b, /count[\s\S]*0/, /return[\s\S]*ok\s*:\s*false/)
    return { ok: counted && branch, detail: `update-count:"exact"=${counted} refuse-branch=${branch}` }
  },
})
