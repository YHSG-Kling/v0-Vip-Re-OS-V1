...
async function testVendorGateway() {
  ...
  check("domainsForScope: agent sees the full provider-backed set; IDX/listing is platform-only", (() => { const ds = domainsForScope("agent"); return ds.includes("email") && ds.includes("transaction") && ds.includes("esign") && ds.includes("showing") && !ds.includes("listing") && !ds.includes("documents") && !ds.includes("marketing") })())
  ...
  check("registry: IDX/listing is platform-only (no per-tier providers)", CONNECTOR_PROVIDERS.listing.length === 0 && !isConnectionAllowed("brokerage", "listing") && !isConnectionAllowed("platform", "listing"))
