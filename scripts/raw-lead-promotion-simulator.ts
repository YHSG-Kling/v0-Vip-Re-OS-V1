 ...
 *
 * Proves the admin Raw Leads review + manual-promotion flow:
 *  - PURE (always): rawLeadReviewStatus — which raw records an admin may promote
 *    (pending/error/retry → yes; already-promoted/duplicate → no).
 *  - LIVE (creds-gated, self-cleaning): seeds a tagged brokerage + raw_scraped_leads
 *    row, runs the promotion lib that promoteLead orchestrates
 *    (evaluatePromotionEligibility + promoteRawRecordToLead), asserts a lead row was
 *    created for the brokerage, then deletes every tagged row in reverse FK order.
 *
...
function selfTest(): void {
  ...
  check("pending → promotable", (() => { const s = rawLeadReviewStatus({ processing_status: "pending" }); return s.canPromote && s.tone === "pending" })())
  check("already promoted → not promotable", (() => { const s = rawLeadReviewStatus({ lead_id: randomUUID() }); return !s.canPromote && s.tone === "promoted" })())
  check("duplicate → not promotable", (() => { const s = rawLeadReviewStatus({ dedupe_status: "duplicate" }); return !s.canPromote && s.tone === "duplicate" })())
  check("error → promotable (retry)", (() => { const s = rawLeadReviewStatus({ processing_status: "error", error_message: "x" }); return s.canPromote && s.tone === "error" })())
  check("prior attempt → promotable (retry, labeled)", (() => { const s = rawLeadReviewStatus({ promotion_attempts: 2 }); return s.canPromote && s.tone === "attempted" && /2/.test(s.label) })())
  check("promoted beats every other signal", rawLeadReviewStatus({ lead_id: "L", processing_status: "error", dedupe_status: "duplicate" }).canPromote === false)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log("\n✅ RAW_LEAD_PROMOTION — raw record review→promotion exercised on the live DB.")
  } finally {
