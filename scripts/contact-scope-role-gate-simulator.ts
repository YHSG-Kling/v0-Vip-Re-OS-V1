...
function main() {
  ...
  console.log("    THIS BLIND SPOT IS CLOSED (wave 27): app/actions/ai-insights.ts#draftSmartEmail")
  console.log("    used to return a bare \"\" for all four of its refusals, so it had no sentences")
  console.log("    to tell apart and passed vacuously. It now returns a discriminated")
  console.log("    DraftSmartEmailResult with a distinct reason per outcome — and all three of")
  console.log("    its callers read it — so that assertion now judges it for real.")
  console.log("    A gate that returned a single bare value for every refusal would still pass")
  console.log("    vacuously; nothing here discovers that shape, it is noticed by reading.")
