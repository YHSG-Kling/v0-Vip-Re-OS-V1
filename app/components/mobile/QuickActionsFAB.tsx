...
const QuickActionsFAB: React.FC = () => {
    ...
    const actions = [
        { 
            ...
            icon: Camera, 
            color: 'bg-purple-600',
            onClick: () => router.push('/listings/new?action=photo')
        },
        { 
            label: 'Add Note', 
            icon: PenTool, 
                    ...
                    {actions.map((action, i) => (
                        <div key={i} className="flex items-center gap-3 group">
                            ...
                            <button 
                                ...
                                className={`w-12 h-12 ${action.color} text-white rounded-full shadow-2xl flex items-center justify-center active:scale-90 transition-transform`}
                            >
                                <action.icon size={20} />
                            </button>
                        </div>
