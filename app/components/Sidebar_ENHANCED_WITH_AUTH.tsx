    ...
    <div className="h-screen w-64 bg-slate-900 text-white flex flex-col fixed left-0 top-0 border-r border-slate-800 z-40">
      ...
      {user && (
        <div className="px-4 py-3 bg-slate-800/50 border-b border-slate-700">
          <p className="text-xs font-bold text-white truncate">{getUserDisplayName()}</p>
          <p className="text-[11px] text-slate-400 mt-1">{getRoleLabel()}</p>
          {user.brokerage && (
            <p className="text-[10px] text-slate-500 mt-1">{user.brokerage}</p>
          )}
        </div>
      )}
      ...
      <div className="p-4 border-t border-slate-800">
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold text-slate-400 hover:bg-slate-800 hover:text-white transition-colors"
        >
          <LogOut size={16} />
          Log Out
        </button>
