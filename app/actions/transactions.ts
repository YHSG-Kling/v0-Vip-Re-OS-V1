...
export async function getTransactions(filters?: {
  ...
  // Handle both agent_id and agentId for compatibility
  ...
  if (agentIdValue) {
    query = query.eq("agent_id", agentIdValue)
  }
  if (filters?.brokerage_id) {
...
export async function addTitleEscrow(data: {
  ...
  await addTimelineEntry(
    data.transaction_id,
    "title_escrow_added",
    `Title company "${data.title_company_name}" assigned`,
  )
...
export async function getUpcomingDeadlines(agentId?: string, days = 7) {
  ...
  if (agentId) {
    query = query.eq("transactions.agent_id", agentId)
  }
...
export async function calculateCommissions(transactionId: string) {
  ...
  const salePrice = transaction.contract_price || 0
    ...
    transaction.transaction_commissions?.map(
      ...
      amount = 0
        ...
        if (comm.split_percentage) {
          amount = amount * (comm.split_percentage / 100)
        }
  ...
  // Update each commission with calculated amount
...
export async function respondToRepairRequest(
  ...
  if (data?.transactions?.id) {
    await addTimelineEntry(
      ...
      "repair_response",
      `Repair request ${response}: ${data.item_description}`,
    )
  }
...
export async function getTransactionStats(agentId?: string) {
  ...
  const { count: pendingDocsCount } = await docsQuery
...
import { generateAIJSON } from "./ai-generate"
...
export async function createTransparentTransaction(data: {
  ...
  // Create transaction with transparency score at 100
...
Length: 3-4 paragraphs`
  ...
  return {
    success: true,
    transaction,
    welcomeMessage: welcome.data?.message,
  }
}
...
export async function generateStatusUpdate(transactionId: string) {
  ...
  const recentMilestones = transaction.transaction_milestones
...
Examples:
- "Waiting for seller's repair response. Usually 2-3 days. I'll follow up tomorrow."
- "Appraisal came in at value! Lender in final underwriting (5-7 days). On track for closing."
- "Title found old lien. Common issue, 7-10 days to clear. Will push closing back slightly."
...
export async function detectTransactionIssues(transactionId: string) {
  ...
  const overdueTasks = transaction.task_items?.filter((t: any) => t.status !== "completed" && new Date(t.due_date) < new Date())
...
export async function deliverEducationalContent(transactionId: string, stage: string) {
  ...
  if (!content) {
    return { success: false, message: "No educational content for this stage" }
  }
  ...
  if (existing) {
    return { success: false, message: "Content already delivered" }
  }

  // Save educational moment
  ...
  return {
    success: true,
    content,
    message: `Educational content delivered for ${stage} stage`,
  }
}
...
export async function monitorTransactionHealth(transactionId: string) {
  ...
  const onTrackCount = milestones.filter((m: any) => m.status === "completed" || new Date(m.target_date) > new Date()).length
  const delayedCount = milestones.filter((m: any) => m.status !== "completed" && new Date(m.target_date) < new Date()).length
  ...
  try {
    ...
    await supabase
      ...
      .update({
        ...
        health_status: health.data.overall_health < 50 ? "critical" : health.data.overall_health < 75 ? "at_risk" : "healthy",
      })
      .eq("id", transactionId)
    ...
    console.error("[v0] Transaction health monitoring error:", error)
...
export async function detectTransactionDelays(transactionId: string) {
  ...
  for (const milestone of transaction.transaction_milestones || []) {
    if (new Date(milestone.target_date) < new Date() && milestone.status !== "completed") {
      const daysOverdue = Math.ceil((Date.now() - new Date(milestone.target_date).getTime()) / (1000 * 60 * 60 * 24))
  ...
  if (delays.length === 0) {
    return { success: true, delays: [], impact: null }
  }
...

{
  ...
  "action_items": ["Contact lender about pending documents", "Schedule follow-up inspection"]
}`
...
export async function celebrateMilestone(transactionId: string, milestone: string) {
  ...
  const celebrations: Record<string, any> = {
    offer_accepted: {
      message: "🎉 Your offer was accepted! This is a huge step. Here's what happens next...",
      tone: "excited",
    },
    inspection_passed: {
      message: "Great news! The inspection went well. No major issues found. Moving forward!",
      tone: "reassuring",
    },
    appraisal_at_value: {
      message: "Excellent! The appraisal came in at value. This clears a major hurdle.",
      tone: "positive",
    },
    clear_to_close: {
      message: "🎈 Clear to Close! Your lender has approved everything. Closing date confirmed!",
      tone: "celebratory",
    },
    closed: {
      message: "🏡 Congratulations! The house is officially yours! Welcome home!",
      tone: "triumphant",
    },
  }
  ...
  if (!celebration) {
    return { success: false, message: "No celebration for this milestone" }
  }
  ...
  return {
    success: true,
    celebration,
    message: "Celebration moment recorded",
  }
}
...
export async function loadClientDashboard(transactionId: string, contactId?: string) {
  ...
  // Verify transaction belongs to contact (security check)
  if (contactId && transaction.contact_id !== contactId) {
    throw new Error("Unauthorized: Transaction does not belong to this contact")
  }

  // Detect persona from contact
  const persona = transaction.contacts?.contact_persona || transaction.transaction_type === "sale" ? "seller" : "buyer"
...
function getPersonaConfig(persona: string, transactionType: string) {
  const configs: Record<string, any> = {
    first_time_buyer: {
      ...
      icon: "home",
      welcomeMessage: "We're here to guide you through every step of buying your first home!",
    },
    luxury_buyer: {
      theme: "luxury",
      primaryColor: "purple",
      ...
      icon: "crown",
      welcomeMessage: "White-glove service for your premium property purchase.",
    },
    motivated_seller: {
      theme: "seller",
      primaryColor: "green",
      title: "Fast-Track Home Sale",
      icon: "trending-up",
      welcomeMessage: "Let's get your home sold quickly and for top dollar.",
    },
    investor: {
      theme: "investor",
      primaryColor: "amber",
      ...
      icon: "bar-chart",
      welcomeMessage: "Data-driven insights for your investment portfolio.",
    },
    relocating: {
      theme: "relocation",
      primaryColor: "teal",
      ...
      icon: "map",
      welcomeMessage: "Making your move to a new city seamless.",
    },
  }
  ...
  return (
    ...
    (transactionType === "sale"
      ? {
          theme: "seller",
          primaryColor: "green",
          title: "Your Home Selling Journey",
          icon: "home",
          welcomeMessage: "Let's sell your home for the best price.",
        }
      : {
          theme: "buyer",
          primaryColor: "blue",
          title: "Your Home Buying Journey",
          icon: "home",
          welcomeMessage: "Welcome to your home buying journey!",
        })
  )
}
  ...
  const education: Record<string, Record<string, any>> = {
    first_time_buyer: {
      offer: {
        ...
        content: "Learn what happens after your offer is submitted and how to prepare for next steps.",
        videoUrl: "/education/first-time-buyer-offer.mp4",
        checklist: ["Review offer terms with your agent", "Prepare earnest money deposit", "Stay available for calls from your agent", "Avoid making major purchases or opening new credit"],
      },
      inspection: {
        title: "Home Inspection 101",
        content: "What to expect during your home inspection and how to interpret the results.",
        videoUrl: "/education/inspection-guide.mp4",
        checklist: ["Attend the inspection if possible", "Ask inspector questions", "Review the full report carefully", "Discuss repair requests with your agent"],
      },
      appraisal: {
        title: "Understanding Appraisals",
        content: "How appraisals work and what happens if the value comes in low.",
        videoUrl: "/education/appraisal-explained.mp4",
        checklist: ["Appraiser will visit the home", "Keep property accessible", "Wait 1-2 weeks for report", "Review results with your agent"],
      },
      financing: {
        title: "Final Loan Steps",
        content: "What your lender needs and how to stay on track for closing.",
        videoUrl: "/education/loan-processing.mp4",
        checklist: ["Provide all requested documents promptly", "Don't change jobs or income", "Avoid opening new credit accounts", "Stay in close contact with your lender"],
      },
    },
    motivated_seller: {
      listing: {
        title: "Maximizing Your Sale",
        content: "Quick wins to prepare your home for a fast, profitable sale.",
        videoUrl: "/education/fast-sale-tips.mp4",
        checklist: ["Declutter and depersonalize all rooms", "Deep clean entire home", "Make minor repairs and touch-ups", "Professional photos scheduled"],
      },
      negotiation: {
        title: "Negotiation Strategies",
        content: "How to evaluate offers and negotiate the best deal.",
        videoUrl: "/education/seller-negotiation.mp4",
        checklist: ["Review all offers thoroughly", "Check buyer pre-approval letters", "Consider terms beyond just price", "Respond to offers promptly"],
      },
      inspection: {
        title: "Buyer Inspection Period",
        content: "What buyers look for and how to handle repair requests.",
        videoUrl: "/education/seller-inspection.mp4",
        checklist: ["Keep home clean and accessible", "Make property available for inspection", "Review buyer's inspection report", "Negotiate repairs fairly and reasonably"],
      },
    },
    investor: {
      due_diligence: {
        title: "Investment Property Analysis",
        content: "Key metrics and data points to validate your investment thesis.",
        videoUrl: "/education/investor-due-diligence.mp4",
        checklist: ["Review comparable rental rates", "Analyze cash-on-cash return", "Inspect property thoroughly for hidden issues", "Verify seller's income/expense statements"],
      },
      closing: {
        title: "Investor Closing Checklist",
        content: "Tax considerations and entity setup for your investment property.",
        videoUrl: "/education/investor-closing.mp4",
        checklist: ["Confirm LLC or entity setup complete", "Review 1031 exchange requirements if applicable", "Coordinate with CPA on tax strategy", "Have property management company lined up"],
      },
    },
    luxury_buyer: {
      offer: {
        title: "Luxury Property Negotiations",
        content: "Strategic considerations for high-value property purchases.",
        videoUrl: "/education/luxury-offer-strategy.mp4",
        checklist: ["Review property disclosure carefully", "Consider additional inspections (pool, septic, etc)", "Verify HOA rules and fees", "Evaluate resale potential"],
      },
    },
    military_buyer: {
      offer: {
        title: "VA Loan Offer Process",
        content: "Using your VA benefits effectively in the offer process.",
        videoUrl: "/education/va-loan-offers.mp4",
        checklist: ["Ensure VA funding fee is addressed", "Confirm seller accepts VA financing", "Have COE ready for lender", "Discuss PCS timeline with agent"],
      },
    },
  }
...
// Enhanced function to auto-update milestones based on persona and stage
export async function autoProgressMilestone(transactionId: string, completedMilestone: string) {
  const supabase = await createClient()

  const { data: transaction } = await supabase.from("transactions").select("*, contacts(*)").eq("id", transactionId).single()
  ...
  // Deliver educational content for next stage
  const nextStage = getNextStage(completedMilestone, persona)
  if (nextStage) {
    await deliverEducationalContent(transactionId, nextStage)
  }
...
async function getCostBreakdown(transactionId: string) {
  const supabase = await createClient()
  const { data } = await supabase.from("transaction_cost_breakdown").select("*").eq("transaction_id", transactionId).maybeSingle()
  return data || {}
}

export async function getClientTasks(transactionId: string) {
  const supabase = await createClient()
  const { data } = await supabase.from("tasks").select("*").eq("transaction_id", transactionId).order("due_date", { ascending: true })
  return data || []
}
...
async function getTransactionHealth(transactionId: string) {
  const supabase = await createClient()
  const { data: transaction } = await supabase.from("transactions").select("health_score").eq("id", transactionId).single()
...
function friendlyStageName(status: string, persona: string): string {
  const buyerStages: Record<string, string> = {
  ...
  const sellerStages: Record<string, string> = {
...
async function generateFriendlyStatusMessage(transaction: any, persona: string): Promise<string> {
  ...
  const buyerMessages: Record<string, string> = {
    offer: "Your offer has been submitted! We're waiting to hear back from the seller.",
    inspection: "Time for the home inspection. We'll make sure everything looks good.",
    appraisal: "The appraiser is evaluating the home's value for your lender.",
    financing: "Your loan is being processed. Almost there!",
    clear_to_close: "You're cleared to close! Final walkthrough coming up.",
    closed: "Congratulations! The house is officially yours!",
  }

  const sellerMessages: Record<string, string> = {
    listing: "Your home is live! We're marketing it to qualified buyers.",
    offer: "Great news! You've received an offer. Let's review it together.",
    negotiation: "We're negotiating the best terms for you.",
    inspection: "The buyer is inspecting the property. This is normal.",
    appraisal: "The appraisal is in progress. Fingers crossed!",
    clear_to_close: "You're cleared to close! Almost time to celebrate.",
    closed: "Congratulations! Your home is sold!",
  }
...
function getMilestoneIcon(milestoneName: string): string {
  const icons: Record<string, string> = {
    "Offer Accepted": "check-circle",
    "Home Inspection": "search",
    Appraisal: "dollar-sign",
    "Loan Processing": "file-text",
    "Clear to Close": "check-square",
    Closing: "home",
  }
...
// ============================================
// AGENT DASHBOARD - TRANSACTION MANAGEMENT
// ============================================
...
function identifyAtRiskDeals(transactions: any[]) {
  ...
  return transactions
    ...
    .map((t) => ({
      ...t,
      riskFactors: getRiskFactors(t),
      priority: calculateRiskPriority(t),
    }))
...
function getRiskFactors(transaction: any): string[] {
  ...
  if ((transaction.health_score || 100) < 60) {
...
// Kanban board data for agent view
export async function getAgentTransactionKanban() {
  ...
  const kanbanStages = {
    lead: {
      title: "Leads",
      deals: transactions?.filter((t) => t.status === "lead" || !t.status) || [],
      color: "gray",
    },
    offer: {
      title: "Active Offers",
      deals: transactions?.filter((t) => t.status === "offer" || t.status === "negotiation") || [],
      color: "blue",
    },
    contract: {
      title: "Under Contract",
      deals:
        transactions?.filter((t) => t.status === "inspection" || t.status === "appraisal" || t.status === "financing") ||
        [],
      color: "yellow",
    },
    closing: {
      title: "Closing Soon",
      deals: transactions?.filter((t) => t.status === "clear_to_close") || [],
      color: "green",
    },
  }
