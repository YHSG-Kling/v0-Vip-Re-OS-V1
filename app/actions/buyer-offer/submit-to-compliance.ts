...
export async function submitOfferToCompliance(
  ...
  const { data: offer } = await supabase
    .from("offers")
    .select("id, brokerage_id, contact_id, agent_id, transaction_id, buyer_signed_at, seller_response_type, fully_signed_contract_received_at, ready_for_compliance_at, compliance_passed_at, closing_date, inspection_period_days, appraisal_contingency_days, financing_contingency_days, earnest_money")
    .eq("id", offerId)
    .maybeSingle()
  ...
  if (offer.seller_response_type !== "accepted" || !offer.fully_signed_contract_received_at) {
    return { success: false, error: "Seller has not accepted with a signed contract yet — record the seller response first" }
  }
