...
import { Loader2, UserPlus, ShieldOff, Shield, LifeBuoy, ChevronDown, ChevronUp, FileCheck2 } from 'lucide-react'
import { useToast } from '@/hooks/use-toast'
import {
  createPlatformStaffAction, updatePlatformStaffRoleAction, revokePlatformStaffAction, listPlatformStaffAction,
  upsertStaffProfileAction, getStaffProfilesAction, type StaffProfile,
} from '@/app/actions/superadmin/platform-staff'
...
// Per-staff HR RECORD editor (platform_staff_profiles): title, start date, the
// employment agreement text, when the STAFF MEMBER acknowledged it (set only by
// their own acknowledgeMyAgreementAction — never editable here), and notes.
function StaffProfileEditor({ userId, profile, onSaved }: { userId: string; profile: StaffProfile | null; onSaved: () => void }) {
  const [title, setTitle] = useState(profile?.title ?? '')
  const [startDate, setStartDate] = useState(profile?.start_date ?? '')
  const [agreement, setAgreement] = useState(profile?.employment_agreement_text ?? '')
  const [notes, setNotes] = useState(profile?.notes ?? '')
  const [saving, startSaving] = useTransition()
  const { toast } = useToast()

  const agreementChanged = (agreement.trim() || null) !== (profile?.employment_agreement_text ?? null)

  function save() {
    startSaving(async () => {
      const r = await upsertStaffProfileAction({ userId, title, startDate, employmentAgreementText: agreement, notes })
      if (r.ok) { toast({ title: 'HR profile saved' }); onSaved() }
      else toast({ title: 'Error', description: r.error, variant: 'destructive' })
    })
  }

  return (
    <div className="grid gap-3 md:grid-cols-2 bg-muted/20 rounded-md p-4">
      <div><Label className="text-xs">Title</Label><Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Support Engineer" /></div>
      ...
      <div className="md:col-span-2">
        <Label className="text-xs">Employment agreement</Label>
        <Textarea rows={5} value={agreement} onChange={(e) => setAgreement(e.target.value)} placeholder="Paste the employment agreement text the staff member must acknowledge…" />
        <div className="mt-1 text-[11px] text-muted-foreground flex items-center gap-1.5">
          {profile?.agreement_acknowledged_at ? (
            <><FileCheck2 className="h-3.5 w-3.5 text-emerald-600" />Acknowledged {new Date(profile.agreement_acknowledged_at).toLocaleString()}{agreementChanged && ' — saving a changed agreement resets this (they must re-acknowledge)'}</>
          ) : profile?.employment_agreement_text ? (
            <>Not yet acknowledged — the staff member sees a banner on their platform home until they acknowledge.</>
          ) : (
            <>No agreement on file. Once saved, the staff member is prompted to acknowledge it on their platform home.</>
          )}
        </div>
      </div>
      <div className="md:col-span-2"><Label className="text-xs">Notes (internal)</Label><Textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} /></div>
      <div className="md:col-span-2">
        <Button size="sm" disabled={saving} onClick={save}>
          {saving && <Loader2 className="h-4 w-4 mr-1.5 animate-spin" />}Save HR profile
        </Button>
      </div>
    </div>
  )
...
export function PlatformStaffManager({ initialStaff, initialProfiles, currentUserId }: { initialStaff: Staff[]; initialProfiles: Record<string, StaffProfile>; currentUserId: string }) {
  const [staff, setStaff] = useState<Staff[]>(initialStaff)
  const [profiles, setProfiles] = useState<Record<string, StaffProfile>>(initialProfiles)
  ...
  function reloadProfiles() { getStaffProfilesAction().then((r) => { if (r.ok) setProfiles(r.profiles) }) }
                  ...
                  <Fragment key={s.id}>
                    <tr className="border-b last:border-0">
                      <td className="px-4 py-2 font-medium">
                        {[s.first_name, s.last_name].filter(Boolean).join(' ') || '—'}{isSelf && <span className="ml-1 text-[10px] text-muted-foreground">(you)</span>}
                        {profile?.title && <div className="text-[11px] font-normal text-muted-foreground">{profile.title}</div>}
                      </td>
                      ...
                      <td className="px-4 py-2 text-right">
                        <div className="inline-flex items-center gap-2">
                          <Button size="sm" variant="outline" className="h-7 px-2 text-xs" onClick={() => setExpanded(isExpanded ? null : s.id)} title="HR record">
                            HR {isExpanded ? <ChevronUp className="h-3 w-3 ml-1" /> : <ChevronDown className="h-3 w-3 ml-1" />}
                          </Button>
                          {!isSelf && (
                            <>
                    ...
                    {isExpanded && (
                      <tr className="border-b last:border-0">
                        <td colSpan={5} className="px-4 py-3">
                          <StaffProfileEditor key={profile?.updated_at ?? 'new'} userId={s.id} profile={profile} onSaved={reloadProfiles} />
                        </td>
                      </tr>
                    )}
                  </Fragment>
