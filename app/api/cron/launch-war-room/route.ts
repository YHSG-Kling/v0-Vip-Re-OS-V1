...
import { runLaunchWarRoom } from "@/lib/kernel/launch-war-room"

export async function GET(req: NextRequest) {
  const unauth = verifyCronAuth(req)
  if (unauth) return unauth
  const ctx = await createCronRunContextAction({ cron_name: "launch-war-room", cron_path: "/app/api/cron/launch-war-room/route.ts" })
  ...
  try {
    ...
    for (const b of (rows ?? []) as Array<{ id: string }>) {
      try {
        const r = await runLaunchWarRoom(b.id, {}, supabase) as unknown as Record<string, unknown>
        for (const [k, v] of Object.entries(r)) if (typeof v === "number") agg[k] = (agg[k] ?? 0) + v
      } catch (e: any) { errors.push(`${b.id}: ${e?.message ?? String(e)}`) }
    }
    await recordCronSuccessAction({ context_id: contextId, records_processed: agg["launches"] ?? 0, metadata: { ...agg, errors: errors.slice(0, 10) } }).catch(() => {})
