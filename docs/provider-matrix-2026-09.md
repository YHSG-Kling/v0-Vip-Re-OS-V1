...
| HubSpot CRM | tenant (brokerage's own private-app token) | **SDK** (`@hubspot/api-client@14.0.1`, `lib/providers/hubspot/client.ts`) | `@hubspot/api-client` | Tenant's own HubSpot plan (not platform-metered) | **Adopted wave 71A.** `lib/crm/providers/hubspot.ts::syncContactToHubSpot` (batch-upsert-by-email + plain create) and `lib/crm/import-pull.ts::pullHubSpot` (paged contact pull) both migrated. |
...
| HubSpot | `lib/providers/hubspot/client.ts` | 2 | 0 | `lib/crm/providers/hubspot.ts` (1), `lib/crm/import-pull.ts` (1) |
