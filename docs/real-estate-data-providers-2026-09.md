...
| Address verification (`lib/external/lob-address-verify.ts` survivor; the `verifyAddressBatchData` fallback was deleted in lane 84C with the promotion gate's address arm) | Lob (at the direct-mail send) | Lob `PLATFORM_VENDOR_RATES.lob=$0.84/piece` (mail, different job) — address-verify-only pricing not separately broken out; BatchData address verify billed against the search token | **keep** — Lob is the named survivor (CLAUDE.md §1 tombstone in `lib/external/batchdata-client.ts`); no candidate here publishes a cheaper standalone address-verify SKU |
